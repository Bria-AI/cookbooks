...
from diffusers.pipelines.bria.pipeline_bria import get_t5_prompt_embeds
...
def main(args):
    ...
    vae_config_shift_factor = vae.config.shift_factor if vae.config["shift_factor"] is not None else 0.0
    vae_config_scaling_factor = vae.config.scaling_factor if vae.config["scaling_factor"] is not None else 1.0
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                latent_model_input = torch.cat([packed_noisy_model_input] * 2) if BriaPipeline.do_classifier_free_guidance else packed_noisy_model_input
                if type(noise_scheduler_copy) != FlowMatchEulerDiscreteScheduler:
                    latent_model_input = noise_scheduler_copy.scale_model_input(latent_model_input, timesteps)
...
# python train_lora.py --pretrained_model_name_or_path briaai/BRIA-4B-Adapt --dataset_name example_finetune_data/ --output_dir example_output_lora/ --max_train_steps 1500 --rank 128 --train_batch_size 1 --gradient_accumulation_steps 4
# max_sequence_length can be 256 or 512 as well 